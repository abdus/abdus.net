...
done. For some weird reasons, I thought I would be able to access database as
the `root` user through a non-root process. Here are some notes about that just
in case I encounter such issues again in future.
...
Once the installation process is over, we need to initialize data directory. We
also need to create [system tables](https://mariadb.com/kb/en/system-tables/).
One of these system database is `mysql` which contains the `user`
...
script requires to be run as root (or by the super user).
...
After running the above command, start MySQL as system daemon. To do that,
...
At this point, MySQL server should be up and running. To confirm that it is
...
password mismatch error. That's because, regular user can't access
...
user, first log-in as `root` using `sudo` (or super user).
...
control of this database. Some more SQL commands would be suffecient for that.
...
Time to verify if we can login with the newly created user. To do that, we can
login to MariaDB shell using `mysql -u USERNAME -p` (with or without `sudo`).
