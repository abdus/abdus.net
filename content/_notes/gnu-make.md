...
- when path of file is not absolute, and the file is not found in current dir,
  `make` looks for it in some other places(in following order). 1. dirs **mentioned with -I** flag (`--include-dir`) 2. `prefix/include` 3. `/usr/gnu/include` 4. `/usr/local/include` 5. `/usr/include`
